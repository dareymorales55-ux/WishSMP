...
/**
 * /essenceremove <player> <amount> -- admin-only. Subtracts from essence
 * level via setEssenceLevel(), with the result clamped between FLOOR (-2)
 * and MAX_ESSENCE (5), same as /essenceset.
 */
public class EssenceRemoveCommand implements CommandExecutor {

    private final PlayerDataManager dataManager;
    private final Apply apply;

    public EssenceRemoveCommand(PlayerDataManager dataManager, Apply apply) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            sender.sendMessage("Usage: /essenceremove <player> <amount>");
            return true;
        }
